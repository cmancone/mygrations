...
class NumericTest(unittest.TestCase):
    ...
    def test_default_errors(self):
        self.assertEquals(
            ...
            Numeric('test', 'DECIMAL', default='').errors
        )
        self.assertEquals(
            [f'Column test of type INT must have an integer value as a default'],
            Numeric('test', 'INT', default=5.0).errors
        )
        self.assertEquals(
            [f'Column test of type BIT must have a default of 1 or 0'],
            Numeric('test', 'BIT', default=5).errors
        )
        self.assertEquals([], Numeric('test', 'BIT', default=1).errors)
        self.assertEquals([], Numeric('test', 'INT', default=5).errors)
        self.assertEquals([], Numeric('test', 'FLOAT', default=5.0).errors)

    def test_length_errors(self):
        self.assertEquals(
            [f'Column test of type FLOAT cannot have a length'],
            Numeric('test', 'FLOAT', length=5).errors
        )
        self.assertEquals(
            [f'Column test of type INT must have an integer value as its length'],
            Numeric('test', 'INT', length='5,2').errors
        )

    def test_character_set_errors(self):
        self.assertEquals(
            [f'Column test of type INT cannot have a character set'],
            Numeric('test', 'INT', character_set='UTF-8').errors
        )
        self.assertEquals(
            [f'Column test of type INT cannot have a collate'],
            Numeric('test', 'INT', collate='UTF-8').errors
        )

    def test_auto_increment_errors(self):
        self.assertEquals(
            [f'Column test of type FLOAT cannot be an AUTO_INCREMENT'],
            Numeric('test', 'FLOAT', auto_increment=True).errors
        )
        self.assertEquals([], Numeric('test', 'FLOAT', auto_increment=False).errors)
        self.assertEquals([], Numeric('test', 'INT', auto_increment=True).errors)
