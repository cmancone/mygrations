...
class TestIndex(unittest.TestCase):
    ...
    def test_can_create(self):
        ...
        self.assertEqual('test_index', index.name)
        self.assertEqual('UNIQUE', index.index_type)
        self.assertEqual(self.columns, index.columns)
        self.assertEqual([], index.schema_errors)
        self.assertEqual([], index.schema_warnings)
        self.assertEqual([], index.parsing_errors)
        self.assertEqual([], index.parsing_warnings)

    def test_string_conversion(self):
        index = Index(name='test_index', columns=self.columns, index_type='UNIQUE')

        self.assertEqual('UNIQUE KEY `test_index` (`name`,`description`)', str(index))
    ...
    def test_name_too_long(self):
        ...
        self.assertEqual([f'Key name {too_long} must be <=64 characters long'], index.schema_errors)

    def test_missing_name_use_column(self):
        index = Index(name='', columns=['hey'], index_type='sup')
        self.assertEqual('hey', index.name)
